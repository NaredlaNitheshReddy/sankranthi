package com.sankranthi.ledger.ui.admin
...
import com.sankranthi.ledger.data.model.Role as AppRole
...
fun AdminScreen(
    ...
    onSetRole: (Profile, AppRole) -> Unit,
    onDismissError: () -> Unit,
    ...
    Card(modifier.fillMaxWidth()) {
        Column(
            ...
            Permission.entries.forEach { permission ->
                val checked = selected[permission] == true
                // The whole row toggles, not just the box: a 20dp checkbox is a
                // poor tap target, and the label reading as dead space is worse.
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
                        .toggleable(
                            value = checked,
                            role = Role.Checkbox,
                            onValueChange = { selected[permission] = it },
                        ),
    ...
    Card(modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
            ...
            if (profile.isAdmin) {
                ...
                Permission.entries.forEach { permission ->
                    val granted = permission in profile.grantedPermissions
                    // Row-level toggle, matching the approval card above.
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                            .toggleable(
                                ...
                                role = Role.Switch,
                                onValueChange = { onTogglePermission(permission, it) },
                            ),
                        horizontalArrangement = Arrangement.SpaceBetween,
                        verticalAlignment = Alignment.CenterVertically,
                    ) {
            ...
            Row(
                ...
                if (!isSelf) {
                    TextButton(
                        onClick = { onSetRole(if (profile.isAdmin) AppRole.MEMBER else AppRole.ADMIN) },
                    ) {
